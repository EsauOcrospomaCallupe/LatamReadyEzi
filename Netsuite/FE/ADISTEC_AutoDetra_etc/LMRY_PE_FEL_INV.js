	...
	function send(plugInContext) {	
		...
		try {
			...
			for (var j = 1; j < COD.length; j=j+2) {
				...
				if(COD[j] != null && COD[j] != ''){
					if(COD[j] == 'IGV_PE:S-PE'){
						reg = new RegExp('-COD-IGV_PE:S-PE-COD-', "g");
						xmlEnvio = xmlEnvio.replace(reg, '10');
					}
					if(COD[j] == 'IGV_PE:E-PE'){
						reg = new RegExp('-COD-IGV_PE:E-PE-COD-', "g");
						xmlEnvio = xmlEnvio.replace(reg, '20');
					}
					if(COD[j] == 'IGV_PE:X-PE'){
						reg = new RegExp('-COD-IGV_PE:X-PE-COD-', "g");
	...
	function getEnableFeatures(){
        ...
        busqEnabFet = search.create({
            ...
            columns: ['custrecord_lmry_pe_fel_usuario_ws', 'custrecord_lmry_pe_fel_password_ws', 
                     ...
                     'custrecord_lmry_pe_auto_detra','custrecord_lmry_pe_codigo_detra2']
        });
        ...
        if(resultEnabFet != null && resultEnabFet.length != 0){
          ...
          AUTODETRA = resultEnabFet[0].getValue(row[10]);
          CODIGODETRA2 = resultEnabFet[0].getValue(row[11]);
        }
    }
