	...
	function send(plugInContext) {
		...
		try {
			...
			*/
            ...
            var descGlo = xmlEnvio.split('-MONTO2005-')[1];

            var desclast = '-MONTO2005-'+descGlo+'-MONTO2005-';
            descGlo=parseFloat(descGlo);
            var descxn=0.00;
            var descxy=0.00;
			...
			*/
			...
			for (var i = 0; i < numLines-1; i++) {
				...
				if(resultItemDescxn != null && resultItemDescxn.length != 0 && resultItemDescxy != null && resultItemDescxy.length != 0){
						...
						if (inv_lines[i][0] == num_linea) {
							if(inv_lines[i][5]== 'x+n'){
								...
								if(montodesc != null && montodesc != ''){
									...
									descxn=descxn+parseFloat(montodesc);
								...
								if(inv_lines[i][5]== 'x+y'){
									...
									if(monto2 != null && monto2 != ''){
										...
										descxy=descxy+parseFloat(monto2);
			...
			*/
			...
			xmlEnvio = xmlEnvio.replace('-1003-', monto_1003.toFixed(2));
         	xmlEnvio = xmlEnvio.replace('-2005-', descGlo);
