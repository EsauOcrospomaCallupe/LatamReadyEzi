...
var nameScript = "LMRY_PE_EI_BOL_MAS1_CLNT"

function lmryEIBolMasInitPageCLNT(){
	/*
...
function lmryEIBolMasSaveRecordCLNT(){
	...
	if (param_date_from != null && param_date_from != '' && param_date_to != null && param_date_to != '' && param_serie != null && param_serie != '') {
		...
		busquedaTransacciones.addFilter(new nlobjSearchFilter('custrecord_lmry_pe_ei_status', null, 'isnot', 'Procesando'));

		busquedaTransacciones.addFilter(new nlobjSearchFilter('custrecord_lmry_pe_ei_status', null, 'isnot', 'AUTORIZADO'));
      
      busquedaTransacciones.addFilter(new nlobjSearchFilter('custrecord_lmry_pe_ei_status', null, 'isnot', 'RECHAZADO'));
      busquedaTransacciones.addFilter(new nlobjSearchFilter('custrecord_lmry_pe_ei_status', null, 'isnot', 'ERROR'));
		...
		if(busquedaTransaccionesResult == null || busquedaTransaccionesResult.length == 0){
			...
			return confirm('Este proceso enviará transacciones según criterios ingresados.\n'+"Desea continuar?");
		}
	}
	return true;
 ...
 * @returns {Void}
 */
function lmryEIBolMasFieldChangedCLNT(type, name, linenum){
	//alert('Type: '+type+'; Name: '+name);
	return true;
}

function lmryEIBolMasValidateFieldCLNT(type, lblname, linenum) {
