 ...
 */
    ...
    function lmry_ei_bol_mas(type)
    {
        try{
            ...
            var param_date_from   = objContext.getSetting('SCRIPT', 'custscript_lmry_pe_ei_date_fro');
            var param_date_to     = objContext.getSetting('SCRIPT', 'custscript_lmry_pe_ei_dat_to');
            var param_serie       = objContext.getSetting('SCRIPT', 'custscript_lmry_pe_ei_series');
            //Lote de Lotes Papus
            var lote_lote         = objContext.getSetting('SCRIPT', 'custscript_lmry_pe_ei_ejecu');
            if(lote_lote=='' || lote_lote==null || lote_lote=='null'){
                nlapiLogExecution('ERROR','Primero','EntroPrimerito');
                ...
                var rowLote  = resultsearchLote[0].getAllColumns();
                    ...
                    param_lote=parseInt(lote_lote)+1;
              nlapiLogExecution('ERROR','param_lote',param_lote);
              

            }else{
               param_lote=lote_lote;
            }
            //Para la Subsi
            var subsidiary   = null;
            var subsi_numdoc = "";
            if (featuresubs == true || featuresubs == 'T'){
                param_subsidiary = objContext.getSetting('SCRIPT', 'custscript_lmry_pe_ei_sub');
            ...
            nlapiLogExecution('ERROR','param_date_from - param_date_to - param_serie', param_date_from + ' -'  + param_date_to + ' -'  + param_serie+'-'+param_lote);
            ...
            */
            ...
            if(param_numComprobante=='' || param_numComprobante==null || param_numComprobante=='null' ){
                ...
                nlapiLogExecution('ERROR','nroCbteMain', nroCbte);
            ...
            if(nroCbte=='' || nroCbte==null || nroCbte=='00000000'){
                return true;
            }
            ...
            busqBolMas.addFilter(new nlobjSearchFilter('custrecord_lmry_pe_ei_status', null, 'isnot', 'ERROR'));
            busqBolMas.addFilter(new nlobjSearchFilter('custrecord_lmry_pe_ei_status', null, 'isnot', 'RECHAZADO'));
            ...
            { 
                nlapiLogExecution('ERROR','SALDRA EN :','3...2...1..(.l.)'+id_BMlog);
              ...
              nlapiLogExecution('ERROR','Reemplazo','Lo Reemplazo');
                if(param_finDeploy==2){
                    var params = {
                            custscript_lmry_pe_status_lote: param_lote
                            
                        }
                    nlapiScheduleScript('customscript_lmry_pe_ei_bm_status_schdl','customdeploy_lmry_pe_ei_bm_status_schdl',params);
                }
                return true;
            ...
            while(!busqStop){
                    ...
                    for ( var i = 0; i < resultBolMas.length; i++){
                        ...
                        var terminos = resultBolMas[i].getText(row[23]);
                        var fechaVenc = resultBolMas[i].getValue(row[24]);
                            //fechaVenc =FormatoDDMMYYY(fechaVenc);
                        var memo = resultBolMas[i].getValue(row[25]);
                        ...
                        while(k <= cant_id){
                            ...
                            if (k == cant_id) {
                                ...
                                xmlEnvio +=
                                    ...
                                    '\t\t<TextoLibre3>'+terminos+'</TextoLibre3>\n' +
                                    '\t\t<TextoLibre4>'+fechaVenc+'</TextoLibre4>\n' +
                                    '\t\t<TextoLibre5></TextoLibre5>\n' + 
                                    '\t\t<TextoLibre6></TextoLibre6>\n' + 
                                    '\t\t<TextoLibre7></TextoLibre7>\n' + 
                                    '\t\t<TextoLibre8></TextoLibre8>\n' + 
                                    '\t\t<TextoLibre9>'+memo+'</TextoLibre9>\n' +
                                    '\t\t<TextoLibre10></TextoLibre10>\n' + 
                                    '\t\t<TextoLibre11></TextoLibre11>\n' + 
                                    '\t\t<TextoLibre12></TextoLibre12>\n' + 
                                    '\t\t<TextoLibre13></TextoLibre13>\n' + 
                                    '\t\t<TextoLibre14></TextoLibre14>\n' + 
                                    '\t</TextosLibres>\n'+
           ...
           // log.error("WSAutCbtsAsinc", "WSAutCbtsAsinc");
            ...
            if (returnAutCbtsAsinc == 'Error') {
                //mailResumen();
                 nlapiLogExecution('ERROR','ASINCRONICO','ERROR');
                return true;
            }
            //return true;
            ...
            if(flag_schdl){
                for (var ez = 1; ez < 50; ez++) {
                    param_numComprobante=param_numComprobante+1000;
                    var auxiliar="" +param_numComprobante;
                    var pads= "00000000";
                    auxiliar= pads.substring(0, pads.length - auxiliar.length) + auxiliar;
                    if(ez==49){
                        var params = {
                            custscript_lmry_primer_comprobante: auxiliar,
                            custscript_lmry_estado_schdl: '2',
                            ...
                            custscript_lmry_pe_ei_ejecu: param_lote
                        }
                    }else{
                        var params = {
                            custscript_lmry_primer_comprobante: auxiliar,
                    ...
                    var deploy="customdeploytest"+ez;
                    nlapiScheduleScript('customscript_lmry_pe_ei_bol_mas_env_schd',deploy,params);
                }
            }
            ...
            num=param_numComprobante;

            for(var er = 0; er < resultBolMas.length; er++){
                var row2  = resultBolMas[i].getAllColumns();
                var id_doc  = resultBolMas[i].getValue(row2[1]);
                var params= new Array();
                    params[0]=TransaccionId;
                    params[1]='ERR_TIM_OUT_COMFIAR';
                    params[2]='Error: El host al que esta intentando conectarse no responde';
                var auxnum = "" + num;
                var pad = "00000000";
                    auxnum = pad.substring(0, pad.length - auxnum.length) + auxnum;
                    params[3]=auxnum;
