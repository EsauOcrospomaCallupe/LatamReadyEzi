				...
				var montoTot2 =0;
					for(var cuenta=1; cuenta<=cantLineas; cuenta++){
						...
						if(variable.getLineItemValue('apply','apply',cuenta)=='T'){
							nlapiLogExecution('ERROR', 'DOCUMENTO APLICADO', variable.getLineItemValue('apply', 'internalid', cuenta));
	...
	//if ( request.getMethod() == 'GET' ) {
		...
		if (featuresubs == true) {
			if (IdSubsidiary != '' && IdSubsidiary != null) {
				IsAgente = nlapiLookupField('subsidiary', IdSubsidiary, 'custrecord_lmry_agente_de_retencion_2');
			}
		}
		else
