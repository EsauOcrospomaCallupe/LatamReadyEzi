 ...
 * File : LMRY_Law360_SOAofTimeSheetSTLT.js
 */
var objContext = nlapiGetContext(); 
// Titulo del Suitelet
//var namereport = "LatamReady - Law360 Rate Assign";
var namereport = "LatamReady - Law360 Ejecucion de Estados de Cuenta de Tiempo";
var LMRY_script = "LatamReady - Law360 Statements Account STLT";
 ...
 */
function suitelet_law360_staacc_main(request, response){
	try {
		if ( request.getMethod() == 'GET' )
		{
			...
			var listalog = form.addSubList('custpage_lmry_log_sublista','staticlist','Log de Procesos','custpage_lmry_log_maintab');
				listalog.addField('custoage_lmry_law360_soalog_dat'	, 'text', 'Fecha de creacion').setDisplayType("disabled");
				if (featuresubs == true || featuresubs == 'T') {
					listalog.addField('custpage_lmry_law360_soalog_sub'	, 'text', 'Subsidiaria').setDisplayType("disabled");
				}
				listalog.addField('custpage_lmry_law360_soalog_per'	, 'text', 'Periodo de Facturacion').setDisplayType("disabled");
				listalog.addField('custpage_lmry_law360_soalog_pro'	, 'text', 'Parametro').setDisplayType("disabled");
				listalog.addField('custpage_lmry_law360_soalog_emp'	, 'text', 'Creado por').setDisplayType("disabled");
				listalog.addField('custpage_lmry_law360_soalog_sta'	, 'text', 'Status').setDisplayType("disabled");
				listalog.addField('custpage_lmry_law360_soalog_end'	, 'text', 'Fecha Termino').setDisplayType("disabled");
				listalog.addRefreshButton();
	
			// the records to be displayed are from a saved search
			var s = nlapiLoadSearch('customrecord_lmry_law360_sta_acc_tim_log', 'customsearch_lmry_law360_sta_acc_tim_log');
			...
			for ( var i = 0; results != null && i < results.length; i++) 
			{
				...
				listalog.setLineItemValue('custoage_lmry_law360_soalog_dat', row, searchresult.getValue('created'));
				if (featuresubs == true || featuresubs == 'T') {
					listalog.setLineItemValue('custpage_lmry_law360_soalog_sub', row, searchresult.getText('custrecord_lmry_law360_soalog_sub'));
				}
				listalog.setLineItemValue('custpage_lmry_law360_soalog_per', row, searchresult.getText('custrecord_lmry_law360_soalog_per'));
				listalog.setLineItemValue('custpage_lmry_law360_soalog_pro', row, searchresult.getText('custrecord_lmry_law360_soalog_pro'));
				listalog.setLineItemValue('custpage_lmry_law360_soalog_emp', row, searchresult.getText('custrecord_lmry_law360_soalog_emp'));
				listalog.setLineItemValue('custpage_lmry_law360_soalog_sta', row, searchresult.getValue('custrecord_lmry_law360_soalog_sta'));
				listalog.setLineItemValue('custpage_lmry_law360_soalog_end', row, searchresult.getValue('custrecord_lmry_law360_soalog_end'));
			}
		...
		{	
			...
			var record = nlapiCreateRecord('customrecord_lmry_law360_sta_acc_tim_log');
				record.setFieldValue('custrecord_lmry_law360_soalog_sub', request.getParameter('custpage_lmry_law360_sub'));
				record.setFieldValue('custrecord_lmry_law360_soalog_per', request.getParameter('custpage_lmry_law360_per'));
				record.setFieldValue('custrecord_lmry_law360_soalog_pro', request.getParameter('custpage_lmry_law360_asu'));
				record.setFieldValue('custrecord_lmry_law360_soalog_emp', nlapiGetUser() );
				record.setFieldValue('custrecord_lmry_law360_soalog_sta', 'Pendiente');
			...
			var params = new Array();
				...
				if (featuresubs == true || featuresubs == 'T') {
					params['custscript_lmry_law360_param_soa_sub'] = request.getParameter('custpage_lmry_law360_sub');
				}
				params['custscript_lmry_law360_param_soa_per']  = request.getParameter('custpage_lmry_law360_per');
				params['custscript_lmry_law360_param_soa_job']  = request.getParameter('custpage_lmry_law360_asu');
				params['custscript_lmry_law360_param_soa_pos']  = 0;
			// Ejecuta el Shedule
